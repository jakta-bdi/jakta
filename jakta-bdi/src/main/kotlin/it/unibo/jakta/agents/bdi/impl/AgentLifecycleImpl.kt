                ...
                when (nextGoal) {
                    is ActInternally -> {
                        ...
                        if (internalAction == null) {
                            ...
                            if (debugEnabled) {
                                println(
                                    "[${agent.name}] WARNING: ${nextGoal.action.functor} Internal Action not found.",
                                )
                            }
                            ExecutionResult(failAchievementGoal(intention, context))
                        } else {
                    ...
                    is ActExternally -> {
                        ...
                        if (externalAction == null) {
                            ...
                            if (debugEnabled) {
                                println(
                                    "[${agent.name}] WARNING: ${nextGoal.action.functor} External Action not found.",
                                )
                            }
                            ExecutionResult(failAchievementGoal(intention, context))
                        } else {
                ...
                when (solution.isYes) {
                    true ->
                        ExecutionResult(
                            context.copy(
                                intentions =
                                    context.intentions.updateIntention(
