...
internal class BeliefBaseImpl private constructor(
    ...
    override fun add(belief: Belief) =
        when (beliefs.count(belief.rule)) {
    ...
    override fun remove(belief: Belief): RetrieveResult =
        if (beliefs.count(belief.rule) > 0) {
    ...
    override fun toString(): String = beliefs.joinToString { Belief.from(it.castToRule()).toString() }

    companion object {
        private val operatorExtension =
            Theory.of(
