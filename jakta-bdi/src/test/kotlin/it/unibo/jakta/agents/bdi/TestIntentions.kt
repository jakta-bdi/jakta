...
class TestIntentions :
    DescribeSpec({
        ...
        val buySomething = Belief.fromSelfSource(Struct.of("buy", variableX))
        val eatSomething = Belief.fromSelfSource(Struct.of("eat", variableX))
        ...
        describe("An intention") {
            ...
            it("should apply a substitution on the actual Activation Record") {
                ...
                val substitution = Substitution.of(variableX, Atom.of("chocolate"))
                val newIntention =
                    Intention.of(
                        intention.recordStack +
                            ActivationRecord.of(
                                listOf(Achieve.of(Struct.of("clean", variableX))),
                                Struct.of("test"),
                            ),
                    )
                ...
                computedIntention.recordStack.last().goalQueue.forEach {
                    it.value.args.first() shouldBe variableX
                }
            }
        }
