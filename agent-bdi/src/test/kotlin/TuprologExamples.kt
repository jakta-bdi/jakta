...
class TuprologExamples : DescribeSpec({

    val customOperators = OperatorSet(
    ...
    describe("parsing custom fixing operators") {
        it("is possibly by customising a term parser") {
            ...
            val expected = Struct.of("|", Struct.of("~", Atom.of("a")), Struct.of("&", Atom.of("b"), Atom.of("c")))
            println(actual)
            actual shouldBe expected
        }
    }

    val jason2Prolog = object : DefaultTermVisitor<Term>() {
    ...
    describe("passing from jason syntax to prolog syntax") {
        it("requires a term visitor") {
            val struct = Struct.of("|", Struct.of("~", Atom.of("a")), Struct.of("&", Atom.of("b"), Atom.of("c")))
            val actual = struct.accept(jason2Prolog)
            val expected = Struct.of(";", Struct.of("not", Atom.of("a")), Struct.of(",", Atom.of("b"), Atom.of("c")))
            println(actual)
            actual shouldBe expected
        }
    }
